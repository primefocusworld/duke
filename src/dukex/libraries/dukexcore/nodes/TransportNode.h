...
::duke::protocol::Transport MAKE(const ::duke::protocol::Transport::TransportType type //
                                 , const int value = -1 //
    ...
    if (type == ::duke::protocol::Transport::CUE) {
...
public:
    void play() {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::PLAY));
        session()->sendMsg(q);
    ...
    void stop() {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::STOP));
        session()->sendMsg(q);
    ...
    void gotoFrame(size_t _f) {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::CUE, _f));
        session()->sendMsg(q);
    }

    void previousFrame() {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::CUE, -1, true));
        session()->sendMsg(q);
    }
    ...
    void firstFrame() {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::CUE_FIRST));
        session()->sendMsg(q);
    }

    void lastFrame() {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::CUE_LAST));
        session()->sendMsg(q);
    }

    void previousShot() {
        MessageQueue q;
        push(q, MAKE(::duke::protocol::Transport::CUE, -1, true, true));
        session()->sendMsg(q);
    }
