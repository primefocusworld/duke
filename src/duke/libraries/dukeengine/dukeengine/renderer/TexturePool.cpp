...
TexturePool::TextureHolder::TextureHolder(const PoolRequest& request, const ResourcePtr& pResource) :
                m_Request(request), m_pResource(pResource) {
}
...
ScopedTexture::ScopedTexture(TexturePool* const pPool, TexturePool::TextureHolderPtr pRecylingTexture) :
                m_pPool(pPool), m_pRecylingTexture(pRecylingTexture) {
    assert( m_pPool);
}
...
int TexturePool::computeDistance(const PoolRequest::Dimension& requested, const PoolRequest::Dimension& actual) {
    if (requested.x > actual.x || requested.y > actual.y || requested.z > actual.z)
        return boost::integer_traits<int>::const_max;
    const int x = requested.x - actual.x;
    const int y = requested.y - actual.y;
    const int z = requested.z - actual.z;
    return x*x+y*y+z*z;
}
